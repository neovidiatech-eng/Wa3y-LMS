...
export default function SupervisorSidebar({ isOpen, onClose, isCollapsed, setIsCollapsed }: SupervisorSidebarProps) {
  ...
  const resolvePath = (path: string) => {
    if (path === '') return '/supervisor-dashboard';
    return `/supervisor-dashboard/${path}`;
  };
      ...
      >
        ...
        <div className={`p-6 border-b border-gray-200 transition-all ${isCollapsed ? 'px-4' : ''}`}>
          <div className="flex items-center gap-3">
            {settings.logoUrl ? (
              <img src={settings.logoUrl} alt={academyName} className="w-12 h-12 rounded-xl object-contain shrink-0" />
            ) : (
            ...
            <div className={`text-right transition-all duration-300 ${isCollapsed ? 'opacity-0 invisible w-0' : 'opacity-100'}`}>
              <h2 className="text-lg font-bold text-gray-900 line-clamp-1">{academyName}</h2>
              <p className="text-xs text-gray-500">لوحة المشرف</p>
            </div>
          </div>
        </div>
                ...
                {item.subItems ? (
                  <>
                    <button
                      ...
                      title={isCollapsed ? (item.label === 'الملف الشخصي' ? item.label : t(item.label)) : ''}
                    >
                      ...
                      {!isCollapsed && (
                        <>
                          <span className={`text-sm font-medium flex-1 ${language === 'ar' ? 'text-right' : 'text-left'}`}>
                            {item.label === 'الملف الشخصي' ? item.label : t(item.label)}
                          </span>
                          <ChevronDown
                      ...
                      <div className={`${language === 'ar' ? 'mr-8' : 'ml-8'} mt-1 space-y-1`}>
                        {item.subItems.map((subItem) => (
                          ...
                          >
                            ...
                            <span>{subItem.label === 'الملف الشخصي' ? subItem.label : t(subItem.label)}</span>
                          </NavLink>
                        ))}
                      </div>
                    )}
